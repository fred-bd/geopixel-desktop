/*!
  \file geopx-desktop/src/geopixeltools/qt/tools/UpdateClass.cpp
...
geopx::tools::UpdateClass::UpdateClass(te::qt::widgets::MapDisplay* display, const QCursor& cursor, te::map::AbstractLayerPtr layer, QObject* parent)
...
geopx::tools::UpdateClass::~UpdateClass()
{
...
bool geopx::tools::UpdateClass::eventFilter(QObject* watched, QEvent* e)
{
...
void geopx::tools::UpdateClass::setLayer(te::map::AbstractLayerPtr layer)
{
  m_layer = layer;
}

void geopx::tools::UpdateClass::selectObjects(QMouseEvent* e)
{
  if (!m_layer.get())
    return;
...
void geopx::tools::UpdateClass::updateClassObjects()
{
  if (!m_layer.get())
    return;
  ...
  {
    // Gets the dataset
    std::unique_ptr<te::da::DataSet> dataset = m_layer->getData(m_roots);
...
void geopx::tools::UpdateClass::drawSelecteds()
{
  if (!m_layer.get())
    return;
  ...
  try
  {
    if (m_roots)
    {
      std::unique_ptr<te::da::DataSet> dsCoords = m_layer->getData(m_roots);
...
void geopx::tools::UpdateClass::cancelOperation()
{
  // Clear draft!
  QPixmap* draft = m_display->getDraftPixmap();
  draft->fill(Qt::transparent);
...
bool geopx::tools::UpdateClass::panMousePressEvent(QMouseEvent* e)
{
...
bool geopx::tools::UpdateClass::panMouseMoveEvent(QMouseEvent* e)
{
  if (!m_panStarted)
    return false;
...
bool geopx::tools::UpdateClass::panMouseReleaseEvent(QMouseEvent* e)
{
  m_panStarted = false;
